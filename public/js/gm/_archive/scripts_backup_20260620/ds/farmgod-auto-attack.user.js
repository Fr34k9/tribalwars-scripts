...
// @author       You
// @match        https://*.staemme.ch/game.php?village=*&screen=am_farm
...
(function () {
    ...
    window.FarmGod.Main = (function (Library, Translation) {
        ...
        const FG_REFRESH_STORAGE_KEY = 'farmGod_auto_refresh_settings';

        const loadRefreshSettings = () => {
            return JSON.parse(localStorage.getItem(FG_REFRESH_STORAGE_KEY)) || {
                enabled: false,
                interval: 600
            };
        };

        const saveRefreshSettings = (settings) => {
            localStorage.setItem(FG_REFRESH_STORAGE_KEY, JSON.stringify(settings));
        };
            ...
            if (game_data.features.Premium.active && game_data.features.FarmAssistent.active) {
                if (game_data.screen == 'am_farm') {
                    $.when(buildOptions()).then((html) => {
                        ...
                        setTimeout(() => {
                            $('.btn.optionButton').trigger('click');
                        }, 0);
                        ...
                        $('.optionButton').off('click').on('click', () => {
                            ...
                            getData(optionGroup, optionNewbarbs, optionLosses).then((data) => {
                                ...
                                $('#fg_auto_refresh').prop('checked', saved.enabled);
                                $('#fg_refresh_interval').val(saved.interval);

                                if (saved.enabled) {
                                    scheduleRefresh();

                                    // ⬇️ resume auto clicking after reload
                                    setTimeout(autoClickFarms, randomBetween(2000, 5000));
                                }

                                bindEventHandlers();

                                $('#fg_auto_refresh').on('change', function () {
                                    const settings = loadRefreshSettings();
                                    settings.enabled = this.checked;
                                    saveRefreshSettings(settings);

                                    if (this.checked) {
                                        scheduleRefresh();

                                        // ⬇️ ONLY start auto clicking when enabled
                                        setTimeout(autoClickFarms, randomBetween(2000, 5000));
                                    } else {
                                        clearTimeout(refreshTimer);
                                        clearInterval(refreshCountdown);
                                        $('#fg_next_refresh').text('');
                                    }
                                });

                                $('#fg_refresh_interval').on('change', function () {
                                    const settings = loadRefreshSettings();
                                    settings.interval = parseInt(this.value, 10) || 600;
                                    saveRefreshSettings(settings);

                                    if ($('#fg_auto_refresh').prop('checked')) {
                                        scheduleRefresh();
                                    }
                                });
        ...
        const detectBot = function () {
            ...
            if ($('#botprotection_quest').length > 0) botDetected = true;
            if ($('#bot_check .btn-default:contains("Starte Botcheck")').length > 0) botDetected = true;
            if (document.body.innerText.indexOf('Farm-Assis') < 0) botDetected = true;

            if (botDetected) {
                console.warn('FarmGod: Bot check detected! Halting automation.');
                if (window.UI) UI.ErrorMessage('BOT CHECK DETECTED! Stopping automation.', 10000);

                fetch('https://ntfy.sh/fr34k_app_logs', {
                    method: 'POST',
                    body: 'Bot check detected in FarmGod automation!',
                    headers: {
                        'Title': 'Tribal Wars Bot Check',
                        'Priority': 'high',
                        'Tags': 'warning,robot'
                    }
        ...
        const scheduleRefresh = () => {
            ...
            const base = parseInt($('#fg_refresh_interval').val(), 10) || 600;
            const jitter = randomBetween(-0.3 * base, 0.3 * base);
            const next = Math.round(base + jitter);

            let remaining = next;
            $('#fg_next_refresh').text(`↻ ${remaining}s`);

            refreshCountdown = setInterval(() => {
                if (detectBot()) {
                    clearTimeout(refreshTimer);
                    clearInterval(refreshCountdown);
                    $('#fg_next_refresh').text(`HALTED`);
                    return;
                }
                remaining--;
                $('#fg_next_refresh').text(`↻ ${remaining}s`);
        ...
        const autoClickFarms = function () {
            const clickNext = () => {
                ...
                const $firstIcon = $('.farmGod_icon').first();
                if (!$firstIcon.length) return;

                const delay = randomBetween(100, 500);
        ...
        const buildTable = function (plan) {
            let html = `<div class="vis farmGodContent"><h4 style="display:flex;align-items:center;gap:10px;">
  FarmGod
  <label style="font-size:11px;">
    <input type="checkbox" id="fg_auto_refresh">
    Auto
  </label>
  <input
    type="number"
    id="fg_refresh_interval"
    value="600"
    min="60"
    style="width:70px;font-size:11px;"
  >
  <span id="fg_next_refresh" style="font-size:11px;color:#555;"></span>
</h4><table class="vis" width="100%">
