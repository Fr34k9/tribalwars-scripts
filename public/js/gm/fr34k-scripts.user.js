...
(function () {
  ...
  function run$6() {
    ...
    $("#nbf-find").on("click", async function() {
      ...
      try {
        ...
        filtered.sort(
          (a, b) => calcDistance(centerCoord, a.x + "|" + a.y) - calcDistance(centerCoord, b.x + "|" + b.y)
        );
        if (filtered.length === 0) {
          ...
          $("#nbf-table-wrap").show();
          utils2.finishScript(filtered.length);
        }
      } catch (err) {
        setStatus("Error: " + err.message);
        utils2.logMessage("Error: " + err, "error");
      }
    ...
    async function getAllFarmCoords() {
      ...
      while (page < totalPages) {
        ...
        utils2.logMessage(`Farm page ${page + 1}/${totalPages} fetched`, "debug");
        page++;
        if (page < totalPages) await utils2.sleep(400);
      }
      return coords;
    }
    async function getBarbarianVillages() {
      ...
      utils2.logMessage(`Found ${barbs.length} barbarian villages in world data`, "debug");
      return barbs;
    }
  ...
  function run() {
    const utils2 = new Fr34kUtils({ script_name: "attack_planner" });
    ...
    const OFF_BOMB_MIN = { axe: 500, light: 200, ram: 30 };
    ...
    const ALL_UNITS = game_data.units && game_data.units.length ? game_data.units : ["spear", "sword", "axe", "archer", "spy", "light", "marcher", "heavy", "ram", "catapult", "knight", "snob"];
                ...
                <div id="ap-settings-content" style="display:none;">
                    <div class="ap-section-body">
                        <div class="ap-settings-grid">
                            <label class="ap-lbl">${unitIcon(((_a = game_data.units) == null ? void 0 : _a.includes("snob")) ? "snob" : "snob")} Nobles</label>
    ...
    async function onLoadTroops() {
      ...
      try {
        ...
        utils2.saveValue("cachedTroops", JSON.stringify(ownVillages.map((v) => ({ id: v.id, name: v.name, x: v.x, y: v.y, troops: v.troops, points: v.points, isOffensive: v.isOffensive }))));
      } catch (err) {
    ...
    async function onPlan() {
      if (!ownVillages.length) {
        const cached = utils2.getValue("cachedTroops");
        if (cached) {
          try {
      ...
      if (settings.sendNobles) {
        if (nobleVillages.length) {
          ...
          const getPreselect = (v) => {
            ...
            const sorted = (pool.length ? pool : reachable.length ? reachable : targets).slice().sort(
              (a, b) => Math.sqrt((v.x - a.x) ** 2 + (v.y - a.y) ** 2) - Math.sqrt((v.x - b.x) ** 2 + (v.y - b.y) ** 2)
            );
            return ((_a = sorted[0]) == null ? void 0 : _a.coords) || "";
          };
    ...
    function onComputePlan() {
      ...
      try {
        ...
        utils2.finishScript(plan.length);
    ...
    async function fetchWorldConfig() {
      ...
      try {
        ...
        utils2.logMessage("World config failed: " + e, "warn");
      }
    }
    async function fetchOwnVillages() {
      ...
      while (page < totalPages) {
        ...
        if (!table) {
          page++;
          if (page < totalPages) await utils2.sleep(300);
          continue;
        }
        table.querySelectorAll("tbody.row_marker").forEach((tbody) => {
          ...
          ALL_UNITS.forEach((u, i) => {
            var _a, _b;
            troops[u] = parseInt((_b = (_a = items[i]) == null ? void 0 : _a.textContent) == null ? void 0 : _b.trim()) || 0;
          });
    ...
    async function resolveTargets() {
      const mode2 = $('input[name="ap-target-mode"]:checked').val();
      const input = $("#ap-targets-input").val().trim();
      if (!input) return [];
      return mode2 === "coords" ? resolveByCoords(input) : resolveByPlayers(input);
    }
    async function resolveByCoords(text) {
      ...
      const targets = [], seen = /* @__PURE__ */ new Set();
      text.split("\n").forEach((line) => {
      ...
      timeframes.forEach(({ from, to }) => {
        ...
        if (settings.sendNobles) {
          Object.entries(assignments.nobles).forEach(([vidStr, tCoords]) => {
            ...
            for (let i = 0; i < snobsLeft; i++) {
              ...
              const isFirst = i === 0;
              const arrivalMs = isFirst ? baseArrivalMs : prevArrivalMs + Math.floor(Math.random() * (NOBLE_DELAY_MAX - NOBLE_DELAY_MIN + 1)) + NOBLE_DELAY_MIN;
              ...
              if (sendTime <= now || isInSleepWindow()) continue;
              let units;
              if (isFirst) {
        ...
        if (settings.sendOffBombs) {
          ...
          bombTargetOrder.forEach((target) => {
            const tCoords = target.coords;
            villages.map((v) => ({ v, tms: calcTravelMs(v, target, OFF_UNITS) })).filter(({ v, tms }) => {
        ...
        if (settings.sendFakes && settings.fakeUnits.length) {
          const fakeTargetOrder = settings.focusEnabled ? [...nonFocusTargets, ...focusTargets] : targets;
    ...
    function buildOffBomb(rt) {
      for (const [u, min] of Object.entries(OFF_BOMB_MIN)) {
        if ((rt[u] || 0) < min) return null;
      }
    ...
    function renderUnitIcons(units, leadUnit) {
      const order = leadUnit ? [leadUnit, ...UNIT_DISPLAY_ORDER.filter((u) => u !== leadUnit)] : UNIT_DISPLAY_ORDER;
    ...
    function savePlan(attacks) {
      try {
        utils2.saveValue("savedPlan", JSON.stringify(attacks.map((a) => ({
          type: a.type,
    ...
    function loadSavedPlan() {
      try {
        const raw = utils2.getValue("savedPlan");
        if (!raw) return null;
    ...
    function saveSettings() {
      utils2.saveValue("sendNobles", $("#ap-nobles").val());
      utils2.saveValue("sendOffs", $("#ap-offs").val());
      utils2.saveValue("sendOffBombs", $("#ap-off-bombs").val());
      utils2.saveValue("sendFakes", $("#ap-fakes").val());
      utils2.saveValue("maxAttacksTo", $("#ap-max-attacks-to").val());
      utils2.saveValue("maxAttacksFrom", $("#ap-max-attacks-from").val());
      utils2.saveValue("fakeUnits", JSON.stringify($(".ap-fake-unit-cb:checked").map(function() {
        return $(this).val();
      }).get()));
      utils2.saveValue("fakesFromAtt", $("#ap-fakes-from-att").val());
      utils2.saveValue("sendFrom", $("#ap-send-from").val());
      utils2.saveValue("targetMode", $('input[name="ap-target-mode"]:checked').val());
      utils2.saveValue("targetInput", $("#ap-targets-input").val());
      const tfs = [];
      $(".ap-tf-row").each(function() {
        const from = $(this).find(".ap-tf-from").val(), to = $(this).find(".ap-tf-to").val();
        if (from && to) tfs.push({ from, to });
      });
      utils2.saveValue("timeframes", JSON.stringify(tfs));
      utils2.saveValue("focusEnabled", $("#ap-focus-enabled").is(":checked") ? "true" : "false");
      utils2.saveValue("focusCoords", $("#ap-focus-coords").val());
      utils2.saveValue("focusRadius", $("#ap-focus-radius").val());
    }
    function restoreSettings() {
      const sv = (id, key) => {
        const v = utils2.getValue(key);
        if (v !== null) $(`#${id}`).val(v);
      };
