...
(async function() {
    ...
    if (window.location.href.indexOf('screen=info_village') > 0) {
        ...
        if ($('#content_value table table').first().find('tr td:contains("Stamm")').parent().find('a[href*="info_ally&id=' + allyId + '"]').length > 0) {
            await getTroobsOfMemberVillage(troopsMemberPage);
   ...
   <tbody>
      ...
      <tr>${troops}</tr>
            ...
            utils.logMessage('Error fetching member village data: ' + error, 'error');
        }
    }
    ...
    async function getMemberUrls() {
        ...
        try {
            ...
            utils.logMessage('Error fetching member URLs: ' + error, 'error');
            return [];
        }
    }
})();
